...
<body class="text-center">
    <form class="form-signin" id="admin_login" name="admin_login" method="post" action="job_seeker_login.php">
        <img class="mb-4" src="images/logo2.png" alt="" width="102" height="102">
        <h1 class="h3 mb-3 font-weight-normal">Please sign in</h1><span class="text-muted">{ Welcome to Falcon
            }</span><br><br>
        ...
        <a href="candidate_form.php">Create an Account </a>
...
if( isset( $_POST['submit']  ) && isset( $_POST['captcha'] ) ){
      ...
      if (!empty($_POST['captcha'])) {
            ...
            if($_SESSION['captcha'] == $_POST['captcha']){
                  ...
                  $sql = "SELECT * FROM `job_seekers` WHERE `email` = '$email' AND `password` = '$pass' ";
                  $res = $db->query($sql);
                  $num = $res->num_rows;
            ...
            echo "<script>alert('Enter the Capcha')</script>";
        }

        if(isset($num) > 0){
          
          $_SESSION['email_job_seeker'] = $email;
          //$_SESSION['login'] = true;
